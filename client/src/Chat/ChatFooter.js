...
function ChatFooter({ roomName, roomId, db, firebase, storage }) {
  ...
  const onEnterPress = (e) => {
    if (e.keyCode === 13 && e.shiftKey === false) {
      ...
      const youtubeLink = /^((?:https?:)?\/\/)?((?:www|m)\.)?((?:youtube\.com|youtu.be))(\/(?:[\w\-]+\?v=|embed\/|v\/)?)([\w\-]+)(\S+)?$/; // eslint-disable-line
      const facebookVideoLink = /^https?:\/\/www\.facebook\.com.*\/(video(s)?|watch|story)(\.php?|\/).+$/;
      const vimeoLink = /(http|https)?:\/\/(www\.|player\.)?vimeo\.com\/(?:channels\/(?:\w+\/)?|groups\/([^\/]*)\/videos\/|video\/|)(\d+)(?:|\/\?)/; // eslint-disable-line
      const soundcloudLink = /^https?:\/\/(soundcloud\.com|snd\.sc)\/(.*)$/; // eslint-disable-line
      const dailymotionLink = /^.+dailymotion.com\/(video|hub)\/([^_]+)[^#]*(#video=([^_&]+))?/; // eslint-disable-line
      const urlLink = /^(?:http(s)?:\/\/)?[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\(\)\*\+,;=.]+$/; // eslint-disable-line
